...
6. ⚠️ 중요: 데이터베이스에서 조회된 실제 데이터만 사용하세요
7. ⚠️ CRITICAL: 데이터는 이미 정렬되어 있습니다. 절대로 순서를 바꾸지 마세요!
8. ⚠️ 순위 질문의 경우: 데이터의 순서를 그대로 따라가세요 (1번째 데이터 = 1위, 2번째 데이터 = 2위...)
9. ⚠️ 타율/홈런 등 통계 질문의 경우: 데이터의 순서를 정확히 유지하여 답변하세요
